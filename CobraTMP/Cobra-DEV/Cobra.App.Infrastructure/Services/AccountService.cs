...
namespace Cobra.App.Infrastructure.Services
{
    public class AccountService : IAccountService
    {
        ...
        public async Task<IdentityResult> ConfirmEmail(string etoken, string email)
        {
            ...
            if (user.IsActive && !user.EmailConfirmed)
            {
                ...
                {
                    var result = await _userManager.ConfirmEmailAsync(user.Id, etoken);
                    if (result.Succeeded)
                    {
        ...
        private async Task<bool> SendConfirmationEmail(ApplicationUser user)
        {
            ...
            nvc.Set("etoken", user.EmailConfirmationToken);         // "token" conflicts with "token" parameter added by PayPal in Return URLs
            nvc.Set("email", user.Email);
            var url = new UriBuilder();
            url.Scheme = request.Url.Scheme;
            url.Host = request.Url.Host;
            //url.Path = "Account/ConfirmEmail";
            url.Path = "Payment/PaymentOptions";                    // Redirect user to payment options page before activating account
            url.Query = StringHelpler.ToQueryString(nvc);
            ...
            string body =
                ...
                + "to activate your account and select payment option.<br />"
