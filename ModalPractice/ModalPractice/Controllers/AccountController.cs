...
namespace ModalPractice.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            var errorList = ModelState.Values.SelectMany(x => x.Errors.Select(y => y.ErrorMessage)).ToList();
            if (!ModelState.IsValid)
            {  
                return Json(new {
                    IfSuccess = false,
                    errMsg = "Server Side Validation Failed.",
                    errorList = errorList
                }); // View(model);
            }
            ...
            switch (result)
            {
                ...
                case SignInStatus.Failure: 
                    return Json(new
                    {
                        ...
                        errorList = new List<string> { "Email or Password is incorrect"}
                    }); // View(model);
                default:
                    return Json(new { });
            }
